...
int main(int argc, char* argv[]) {
	...
	while (dfs.eof() == false) {
		...
		for (unsigned int ci = 0; ci < word.length(); ++ci) {
			...
			// Add pairwise factor between (ci-2) and (ci)
			//std::vector<unsigned int> fac_pw_varindex(2);
			fac_pw_varindex[0] = ci-2;
			fac_pw_varindex[1] = ci;
			std::vector<double> data_pw_empty_2;
			Grante::Factor* fac_pw_2 =
				new Grante::Factor(pt_pw, fac_pw_varindex, data_pw_empty_2);
			fg->AddFactor(fac_pw_2);
		}
		...
		fg_inf[fold_id].push_back(new Grante::BeliefPropagation(fg));
		// Full word
		word.clear();
		data.clear();
	}
	...
	for (unsigned int fold = 0; fold < 2; ++fold) {
...
//		mle.AddPrior("letter_unary", new Grante::NormalPrior(1.0, w.size()));
		...
		for (unsigned int n = 0; n < test_data.size(); ++n) {
			...
			Grante::BeliefPropagation tinf(test_data[n].first);
			std::cout<<"TreeInference method"<<std::endl;
	...
	double sdev_sample = std::sqrt(all_acc_squared/2.0 -
		mean_acc*mean_acc);
